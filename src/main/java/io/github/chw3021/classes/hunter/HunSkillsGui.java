...
public class HunSkillsGui extends SkillsGui{
	...
	public void Hunskillsinv(Player p)
	{
		...
		if(p.getLocale().equalsIgnoreCase("ko_kr")) {
			...
			itemset("두개골분쇄", Material.BOOK, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"✬[바람]","🖮🖰 점프 + 공격", "두개골 분쇄를 제외한", "피해량이 50%증가합니다"),hsd.SkullCrusher.getOrDefault(p.getUniqueId(), 0),1,1.1,0.06,50, 5, skillsInv);
			...
			itemset("극악무도", Material.BOOK, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+at,"","모든 기술은 단한번의 공격을 강화합니다","최대충전 상태가 아닐때는", "절반의 피해만 입힙니다","허기에 면역이 됩니다", "최대충전 상태로 공격시", "적의 최대체력의"+ Math.round(5+0.02*at)  +"% 에 해당하는 추가피해를 줍니다","공격력이 증가합니다",ChatColor.BOLD+"X "+BigDecimal.valueOf(1.15*(1+at*0.0576)).setScale(2, RoundingMode.HALF_EVEN)), 7, skillsInv);
			if(Proficiency.getpro(p)<1) {
				...
				itemset("충격", Material.CREEPER_HEAD, 0, 1, Arrays.asList("주변 적들도 참격시킵니다"), 13, skillsInv);
				itemset("공포", Material.SKELETON_SKULL, 0, 1, Arrays.asList("적 처치시 주변적들이 참격합니다"), 14, skillsInv);
				...
				itemset("갈망", Material.WRITTEN_BOOK, 0, 1, Arrays.asList("웅크리기 + num4", "참격과 두개골분쇄의 대기시간이 초기화됩니다","",ChatColor.BOLD+"X 27.1D"), 17, skillsInv);
			...
			itemset("SkullCrusher", Material.BOOK, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"✬[Wind]","🖮🖰 Jump + Hit", "Increases Damage 50%","Excepting for SkullCrusher's"),hsd.SkullCrusher.getOrDefault(p.getUniqueId(), 0),1,1.1,0.06,50, 5, skillsInv);
			...
			if(Proficiency.getpro(p)<1) {
				...
				itemset("Rage", Material.WRITTEN_BOOK, 0, 1, Arrays.asList("Sneaking + num4", "Resets Cooldown of Daze & SkullCrusher","",ChatColor.BOLD+"X 27.1"), 17, skillsInv);
