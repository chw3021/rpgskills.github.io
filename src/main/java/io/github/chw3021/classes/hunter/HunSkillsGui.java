	...
	public void Hunskillsinv(Player p)
	{
		...
		if(p.getLocale().equalsIgnoreCase("ko_kr")) {
			...
			if(Proficiency.getpro(p)<1) {
				...
				itemset("속박(잠김)", Material.STRUCTURE_VOID, 0, 1, Arrays.asList("요구 숙련도: "+ Proficiency.getproexp(p) + "/29315"), 10, Hunskillsinv);
				...
				itemset("뒷목가격", Material.ZOMBIE_HEAD, 0, 1, Arrays.asList("적을 잠시동안 제압합니다"), 9, Hunskillsinv);
				itemset("속박", Material.COBWEB, 0, 1, Arrays.asList("적을 잠시동안 제압합니다"), 10, Hunskillsinv);
			...
			itemset("Backattack", Material.BOOK, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+hsd.Backattack.getOrDefault(p.getUniqueId(), 0),"","+50% Damage when you hit enemy's back", "Higher Explevel gets Better hit detection", "Master LV.1"), 8, Hunskillsinv);
			if(Proficiency.getpro(p)<1) {
				itemset("BackStrike(Locked)", Material.STRUCTURE_VOID, 0, 1, Arrays.asList("Required Proficiency: "+ Proficiency.getproexp(p) + "/29315"), 9, Hunskillsinv);
				itemset("Bind(Locked)", Material.STRUCTURE_VOID, 0, 1, Arrays.asList("Required Proficiency: "+ Proficiency.getproexp(p) + "/29315"), 10, Hunskillsinv);
				...
				itemset("BackStrike", Material.ZOMBIE_HEAD, 0, 1, Arrays.asList("Holds Hit Enemy Shortly"), 9, Hunskillsinv);
				itemset("Bind", Material.COBWEB, 0, 1, Arrays.asList("Holds Hit Enemy Shortly"), 10, Hunskillsinv);
