...
public class PalSkillsGui extends SkillsGui{
	...
	public void PalSkillsinv(Player p)
	{
		...
		if(p.getLocale().equalsIgnoreCase("ko_kr")) {
			...
			itemset("징벌", Material.GOLDEN_AXE, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+psd.Punish.getOrDefault(p.getUniqueId(),0),"",ChatColor.UNDERLINE+"✬[번개]","🖮🖰 점프 + Hit","",ChatColor.BOLD+" X (0.5D + "+BigDecimal.valueOf(psd.Punish.getOrDefault(p.getUniqueId(),0)*1.5).setScale(2, RoundingMode.HALF_EVEN)+")", "Master Lv.50"), 3, skillsInv);
			...
			itemset("보호", Material.BOOK, 0, 1, Arrays.asList("막기도중 파티원들의 받는피해를 절반 감소시킵니다 (패시브)"), 6, skillsInv);
			...
			if(Proficiency.getpro(p)<1) {
				...
				itemset("광명", Material.DAYLIGHT_DETECTOR, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"✬[번개]","🖮🖰 재입력시 광명을 사용합니다","(피해량은 결박 레벨에 비례합니다)","",ChatColor.BOLD+" X (0.9D + "+BigDecimal.valueOf(psd.Restraint.getOrDefault(p.getUniqueId(),0)*0.9).setScale(2, RoundingMode.HALF_EVEN)+")"), 10, skillsInv);
				...
				itemset("수호", Material.ENCHANTED_BOOK, 0, 1, Arrays.asList("공격력과 방어력이 증가합니다", "방패를 들때 전방에서 오는 모든 피해를","막아주는 보호막을 전개합니다"), 16, skillsInv);
			...
			itemset("Protection", Material.BOOK, 0, 1, Arrays.asList("Reduces damage to Party by 50% while Blocking (Passive)"), 6, skillsInv);
			...
			if(Proficiency.getpro(p)<1) {
				...
				itemset("Illumination", Material.DAYLIGHT_DETECTOR, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"✬[Lightning]","🖮🖰 Use Illumination When Use Once More","(Damage Affected By Restraint)","",ChatColor.BOLD+" X (0.9D + "+BigDecimal.valueOf(psd.Restraint.getOrDefault(p.getUniqueId(),0)*0.9).setScale(2, RoundingMode.HALF_EVEN)+")"), 10, skillsInv);
				...
				itemset("Tutelary", Material.ENCHANTED_BOOK, 0, 1, Arrays.asList("Increases Whole Skills Damage & Armor", "Summon barrier that protects you from all damage", "from the front While Raising Shield"), 16, skillsInv);
