					...
					if(ev.getProjectile().getType() == EntityType.FIREWORK_ROCKET)
					{
						...
						if(ar.getFireworkMeta().getDisplayName().equals("Remedying Rocket")) {
							if(rrcooldown.containsKey(p.getName())) // if cooldown has players name in it (on first trow cooldown is empty)
							{
								...
								{
									Bukkit.getPluginManager().callEvent(new SkillUseEvent(p,timer,0,"치유로켓","RemedyingRocket"));
									if(p.getLocale().equalsIgnoreCase("ko_kr")) {
								...
								{
									Bukkit.getPluginManager().callEvent(new SkillUseEvent(p,sec,0,"치유로켓","RemedyingRocket"));
				...
				for(int count = 0 ; count <11; count++) {
					Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(RMain.getInstance(), new Runnable() {
						...
						public void run()
						{
							...
							cloud.setDuration(3);
							for (Entity e : ev.getEntity().getWorld().getNearbyEntities(ev.getEntity().getLocation(), 5, 5, 5))
							{
								if (e instanceof Player)
								{
