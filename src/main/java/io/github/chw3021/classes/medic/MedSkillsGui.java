	...
	public void Medskillsinv(Player p)
	{
		...
		if(p.getLocale().equalsIgnoreCase("ko_kr")) {
			...
			if(Proficiency.getpro(p)<1) {
				...
				itemset("카드세우스 탑", Material.WRITTEN_BOOK, 0, 1, Arrays.asList("웅크리기 + 아이템던지기", BigDecimal.valueOf(20-ssd.Medicine.getOrDefault(p.getUniqueId(),1)*0.3).setScale(2, RoundingMode.HALF_EVEN)+"초당 1의 회복과 포만을 제공합니다"), 17, Medskillsinv);
				...
				itemset("날개슈트", Material.WRITTEN_BOOK, 0, 1, Arrays.asList("달리기 + 아이템던지기", "날개슈트가 활성화되어 있는동안 공격력이 증가합니다(25초)","치유로켓의 대기시간이 없어집니다", "X (1 + 0.007D)"), 26, Medskillsinv);
			}
