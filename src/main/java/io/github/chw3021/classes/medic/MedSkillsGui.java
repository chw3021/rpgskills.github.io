...
public class MedSkillsGui extends SkillsGui {
	...
	public void Medskillsinv(Player p)
	{
		...
		if(p.getLocale().equalsIgnoreCase("ko_kr")) {
				...
				itemset("의학", Material.BOOK, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+ssd.Medicine.getOrDefault(p.getUniqueId(),1),"","공격력과 회복률이 상승합니다","",ChatColor.BOLD+"X 1.1 + "+BigDecimal.valueOf(ssd.Medicine.getOrDefault(p.getUniqueId(),1)*0.4).setScale(2, RoundingMode.HALF_EVEN)), 7, skillsInv);
				...
				itemset("Medicine", Material.BOOK, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+ssd.Medicine.getOrDefault(p.getUniqueId(),1),"","Increse Damage and Healing Rate","",ChatColor.BOLD+"X 1.1 + "+BigDecimal.valueOf(ssd.Medicine.getOrDefault(p.getUniqueId(),1)*0.4).setScale(2, RoundingMode.HALF_EVEN)), 7, skillsInv);
				if(Proficiency.getpro(p)<1) {
