	...
	public void Snipskillsinv(Player p)
	{
		...
		if(p.getLocale().equalsIgnoreCase("ko_kr")) {
			...
			if(Proficiency.getpro(p)<1) {
				...
				itemset("최후의 한발", Material.WRITTEN_BOOK, 0, 1, Arrays.asList("웅크리기 + 아이템던지기","",ChatColor.BOLD+"+ 10.0D"), 17, Snipskillsinv);
				...
				itemset("독수리분대", Material.WRITTEN_BOOK, 0, 1, Arrays.asList("달리기 + 아이템던지기","",ChatColor.BOLD+"40 X 0.5D, 6.5D"), 26, Snipskillsinv);
			}
