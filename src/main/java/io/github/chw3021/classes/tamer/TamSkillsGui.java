...
public class TamSkillsGui extends SkillsGui{
	...
	public void TamSkillsinv(Player p)
	{
		...
		if(p.getLocale().equalsIgnoreCase("ko_kr")) {
			...
			itemset("벌집", Material.BEEHIVE, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+tsd.BeeHive.getOrDefault(p.getUniqueId(), 0),"",ChatColor.UNDERLINE+"✬[맹독]","🖮🖰 점프 + 우클릭","",ChatColor.BOLD+" X "+BigDecimal.valueOf(3*(1+tsd.BeeHive.getOrDefault(p.getUniqueId(), 0)*0.03)*(1+tsd.Taming.getOrDefault(p.getUniqueId(), 0)*0.023)).setScale(2, RoundingMode.HALF_EVEN)+"D", "Master LV.50"), 3, skillsInv);
			...
			if(Proficiency.getpro(p)<1) {
				...
				itemset("거미줄발사", Material.COBWEB, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"✬[맹독]","🖮🖰 재입력시 거미줄발사를 사용합니다","(피해량은 스파이디 레벨에 비레합니다)","",ChatColor.BOLD+" X "+BigDecimal.valueOf(0.6*(1+tsd.Spidey.getOrDefault(p.getUniqueId(), 0)*0.04)*(1+tsd.Taming.getOrDefault(p.getUniqueId(), 0)*0.058)).setScale(2, RoundingMode.HALF_EVEN)+"D"), 10, skillsInv);
				itemset("도약공격", Material.SLIME_BLOCK, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"✬[대지]","🖮🖰 반려동물들이 존재할때 사용시","동물들이 도약공격을 합니다","만약 집중공격대상이 있다면","해당 목표물로 도약합니다",
						"(피해량은 반려동물 레벨에 비레합니다)","",ChatColor.BOLD+"5 X "+BigDecimal.valueOf(0.3*(1+tsd.Pets.getOrDefault(p.getUniqueId(), 0)*0.03)*(1+tsd.Taming.getOrDefault(p.getUniqueId(), 0)*0.058)).setScale(2, RoundingMode.HALF_EVEN)+"D"), 11, skillsInv);
				itemset("혼란", Material.BEE_NEST, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"✬[맹독]","🖮🖰 재사용시 벌들이 주변적을 공격합니다","(피해량은 벌집 레벨에 비레합니다)",""
						,ChatColor.BOLD+"6 X 5 X "+BigDecimal.valueOf(0.2*(1+tsd.BeeHive.getOrDefault(p.getUniqueId(), 0)*0.01)*(1+tsd.Taming.getOrDefault(p.getUniqueId(), 0)*0.058)).setScale(2, RoundingMode.HALF_EVEN)+"D"), 12, skillsInv);
				...
				itemset("판다휩쓸기", Material.ANVIL, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"✬[대지]","🖮🖰 판다가 존재할때 사용시","판다가 휩쓸기공격을 합니다","만약 집중공격대상이 있다면","해당 목표물로 이동합니다",""
						+ "(피해량은 판다 레벨에 비레합니다)","",ChatColor.BOLD+" X "+BigDecimal.valueOf(0.6*(1+tsd.PandaSweep.getOrDefault(p.getUniqueId(), 0)*0.03)*(1+tsd.Taming.getOrDefault(p.getUniqueId(), 0)*0.058)).setScale(2, RoundingMode.HALF_EVEN)+"D"), 14, skillsInv);
				...
				itemset("철골렘/골렘강타", Material.WRITTEN_BOOK, 0, 1, Arrays.asList(
						ChatColor.UNDERLINE+"✬[대지]","🖮🖰 웅크리기 + num4","",ChatColor.BOLD+" X "+BigDecimal.valueOf(20*(1+tsd.Taming.getOrDefault(p.getUniqueId(), 0)*0.08)).setScale(2, RoundingMode.HALF_EVEN)+"D"
						,"","골렘강타는 철골렘이 존재할때만 사용 가능합니다",""
						,ChatColor.BOLD+" X "+BigDecimal.valueOf(10*(1+tsd.Taming.getOrDefault(p.getUniqueId(), 0)*0.058)).setScale(2, RoundingMode.HALF_EVEN)+"D"), 17, skillsInv);
				...
				itemset("닭타기(잠김)", Material.STRUCTURE_VOID, 0, 1, Arrays.asList("요구 숙련도: "+ Proficiency.getproexp(p) + "/155015"), 21, skillsInv);
				...
				itemset("할퀴기", Material.SHEARS, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"✬[대지]","🖮🖰 재입력시 동물들이 할퀴기공격을 합니다","(피해량은 반려동물 레벨에 비레합니다)"
						,"",ChatColor.BOLD+"5 X "+BigDecimal.valueOf(0.5*(1+tsd.Pets.getOrDefault(p.getUniqueId(), 0)*0.035)*(1+tsd.Taming.getOrDefault(p.getUniqueId(), 0)*0.058)).setScale(2, RoundingMode.HALF_EVEN)+"D"), 19, skillsInv);
				itemset("닭타기", Material.CHICKEN_SPAWN_EGG, 0, 1, Arrays.asList("재입력시 닭을 소환하여 위로 날아오릅니다"), 21, skillsInv);
				itemset("크리퍼구름", Material.CREEPER_BANNER_PATTERN, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"✬[맹독]","🖮🖰 폭발후 구름이 생성됩니다","(피해량은 크리퍼폭탄 레벨에 비레합니다)"
						,"",ChatColor.BOLD+"10 X "+BigDecimal.valueOf(0.08*(1+tsd.CreepBomb.getOrDefault(p.getUniqueId(), 0)*0.07)*(1+tsd.Taming.getOrDefault(p.getUniqueId(), 0)*0.058)).setScale(2, RoundingMode.HALF_EVEN)+"D"), 22, skillsInv);
				itemset("발구르기", Material.COBBLESTONE, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"✬[대지]","🖮🖰 재입력시 판다가 발구르기를 합니다","(피해량은 판다 레벨에 비레합니다)"
						,"",ChatColor.BOLD+" X "+BigDecimal.valueOf(0.6*(1+tsd.PandaSweep.getOrDefault(p.getUniqueId(), 0)*0.03)*(1+tsd.Taming.getOrDefault(p.getUniqueId(), 0)*0.058)).setScale(2, RoundingMode.HALF_EVEN)+"D"), 23, skillsInv);
			...
			itemset("PressTheAttack", Material.ARROW, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+tsd.PressTheAttack.getOrDefault(p.getUniqueId(), 0),"","SwapHand(Arrow) or MeleeAtack","The arrow's attack power is the same", "As 5% of The sum of animals' attack power","","Spidey Will Hold Hit Enemy","CreepBomb will Explode Instantly", "Master LV.1"), 0, skillsInv);
			...
			if(Proficiency.getpro(p)<1) {
				...
				itemset("Leap", Material.SLIME_BLOCK, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"✬[Earth]","🖮🖰 Pets will Leap When Use [Pets]","🖮🖰 While Pets Are Existing","If They Have Target, Leap To Target",
						"(Damage Affected By Pets)","",ChatColor.BOLD+"5 X "+BigDecimal.valueOf(0.3*(1+tsd.Pets.getOrDefault(p.getUniqueId(), 0)*0.03)*(1+tsd.Taming.getOrDefault(p.getUniqueId(), 0)*0.058)).setScale(2, RoundingMode.HALF_EVEN)+"D"), 11, skillsInv);
				...
				itemset("PandaSweep", Material.ANVIL, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"✬[Earth]","🖮🖰 Pandy will Sweep When Use [Panda]","🖮🖰 While Panda is Existing","If Panda Have Target, Sweep To Target",""
						+ "(Damage Affected By Panda)","",ChatColor.BOLD+" X "+BigDecimal.valueOf(0.6*(1+tsd.PandaSweep.getOrDefault(p.getUniqueId(), 0)*0.03)*(1+tsd.Taming.getOrDefault(p.getUniqueId(), 0)*0.058)).setScale(2, RoundingMode.HALF_EVEN)+"D"), 14, skillsInv);
				...
				itemset("Chicken(Locked)", Material.STRUCTURE_VOID, 0, 1, Arrays.asList("Required Proficiency: "+ Proficiency.getproexp(p) + "/155015"), 21, skillsInv);
				...
				itemset("Scratch", Material.SHEARS, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"✬[Earth]","🖮🖰 Pets will Scratch Attack When Use Once More","(Damage Affected By Pets)"
						,"",ChatColor.BOLD+"5 X "+BigDecimal.valueOf(0.5*(1+tsd.Pets.getOrDefault(p.getUniqueId(), 0)*0.035)*(1+tsd.Taming.getOrDefault(p.getUniqueId(), 0)*0.058)).setScale(2, RoundingMode.HALF_EVEN)+"D"), 19, skillsInv);
				itemset("Chicken", Material.CHICKEN_SPAWN_EGG, 0, 1, Arrays.asList("Summon Chickens That Carry You"), 21, skillsInv);
