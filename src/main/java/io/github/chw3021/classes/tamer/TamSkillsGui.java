		...
		if(p.getLocale().equalsIgnoreCase("ko_kr")) {
			...
			if(Proficiency.getpro(p)<1) {
				...
				itemset("철골렘/골렘강타", Material.WRITTEN_BOOK, 0, 1, Arrays.asList(
						ChatColor.UNDERLINE+"[대지 계열]","웅크리기 + 아이템던지기","",ChatColor.BOLD+" X "+BigDecimal.valueOf(20*(1+tsd.Taming.getOrDefault(p.getUniqueId(), 0)*0.08)).setScale(2, RoundingMode.HALF_EVEN)+"D"
				...
				itemset("길들여진 드래곤", Material.WRITTEN_BOOK, 0, 1, Arrays.asList(
						ChatColor.UNDERLINE+"[대지 계열]",ChatColor.BOLD+"150 X "+BigDecimal.valueOf(0.1*(1+tsd.Taming.get(p.getUniqueId())*0.058)).setScale(2, RoundingMode.HALF_EVEN)+"D"), 26, Tamskillsinv);
			}
