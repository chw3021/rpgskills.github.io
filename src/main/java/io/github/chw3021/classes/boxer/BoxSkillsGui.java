...
public class BoxSkillsGui extends SkillsGui{
	...
	public void BoxSkillsinv(Player p)
	{
		...
		if(p.getLocale().equalsIgnoreCase("ko_kr")) {
			...
			itemset("반격", Material.CHAINMAIL_HELMET, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+bsd.Counter.getOrDefault(p.getUniqueId(),0),"","적의 공격을 패리나 위빙으로","회피성공시 발동됩니다","",ChatColor.BOLD+" X"+BigDecimal.valueOf(1.35+bsd.Counter.getOrDefault(p.getUniqueId(),0)*0.02).setScale(2, RoundingMode.HALF_EVEN),"Master LV.50"), 5, Boxskillsinv);
