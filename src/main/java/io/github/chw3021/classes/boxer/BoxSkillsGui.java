	...
	public void BoxSkillsinv(Player p)
	{
		...
		if(p.getLocale().equalsIgnoreCase("ko_kr")) {
			...
			if(Proficiency.getpro(p)<1) {
				...
				itemset("촌경", Material.BLACK_GLAZED_TERRACOTTA, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"[바람 계열]","재입력시 촌경를 사용합니다","",ChatColor.BOLD+" X"+BigDecimal.valueOf(0.91*(1+bsd.Jab.getOrDefault(p.getUniqueId(),0)*0.08)).setScale(2, RoundingMode.HALF_EVEN)+"D", "(피해량은 질풍권 레벨에 비례합니다)"), 20, Boxskillsinv);
			...
			if(Proficiency.getpro(p)<1) {
				...
				itemset("OneInchPunch", Material.BLACK_GLAZED_TERRACOTTA, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"[Wind]","Use OneInchPunch When use Once more","",ChatColor.BOLD+" X"+BigDecimal.valueOf(0.91*(1+bsd.Jab.getOrDefault(p.getUniqueId(),0)*0.08)).setScale(2, RoundingMode.HALF_EVEN)+"D", "(Damage Affected By FlickerJab)"), 20, Boxskillsinv);
