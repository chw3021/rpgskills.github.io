...
public class CheSkillsGui extends SkillsGui{
	...
	public void CheSkillsinv(Player p)
	{
		...
		if(p.getLocale().equalsIgnoreCase("ko_kr")) {
			...
			itemset("돌진" , Material.POTION, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+csd.Charge.getOrDefault(p.getUniqueId(),0),"",ChatColor.UNDERLINE+"✬[맹독]","🖮🖰 웅크리기 + 우클릭","돌진중에는 무적상태입니다","재사용시 일찍 종료할수 있습니다","재사용시 일찍 종료할수 있습니다","",ChatColor.BOLD+" X "+BigDecimal.valueOf(0.79*(1+csd.Charge.getOrDefault(p.getUniqueId(),0)*0.078)).setScale(2, RoundingMode.HALF_EVEN)+"D","Master LV.50"), 2, skillsInv);
			...
			if(Proficiency.getpro(p)<1) {
				...
				itemset("환각제", Material.YELLOW_GLAZED_TERRACOTTA, 0, 1, Arrays.asList("재입력시 환각제를 투척합니다"), 18, skillsInv);
				...
				itemset("발광구", Material.GLOW_LICHEN, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"✬[맹독]","🖮🖰 재입력시 발광구를 투척합니다", "(피해량은 슬라임볼 레벨에 비례합니다)","",ChatColor.BOLD+" X "+BigDecimal.valueOf(0.8*(1+csd.SlimeBall.getOrDefault(p.getUniqueId(),0)*0.08)).setScale(2, RoundingMode.HALF_EVEN)+"D"), 23, skillsInv);
			...
			if(Proficiency.getpro(p)<1) {
				...
				itemset("Hallucinogen", Material.YELLOW_GLAZED_TERRACOTTA, 0, 1, Arrays.asList("Throw Hallucinogen When Use Once More"), 18, skillsInv);
				...
				itemset("GlowingBall", Material.GLOW_LICHEN, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"✬[Poison]","🖮🖰 Throw Glowing Ball When Use Once More", "(Damage Affected By SlimeBall)","",ChatColor.BOLD+" X "+BigDecimal.valueOf(0.8*(1+csd.SlimeBall.getOrDefault(p.getUniqueId(),0)*0.08)).setScale(2, RoundingMode.HALF_EVEN)+"D"), 23, skillsInv);
