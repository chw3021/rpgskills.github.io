...
public class CheSkillsGui extends SkillsGui{
	...
	public void CheSkillsinv(Player p)
	{
		...
		if(p.getLocale().equalsIgnoreCase("ko_kr")) {
			...
			itemset("돌진" , Material.POTION, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+csd.Charge.getOrDefault(p.getUniqueId(),0),"",ChatColor.UNDERLINE+"[맹독 계열]","웅크리기 + 우클릭","돌진중에는 무적상태입니다","재사용시 일찍 종료할수 있습니다","재사용시 일찍 종료할수 있습니다","",ChatColor.BOLD+" X "+BigDecimal.valueOf(0.79*(1+csd.Charge.getOrDefault(p.getUniqueId(),0)*0.078)).setScale(2, RoundingMode.HALF_EVEN)+"D","Master LV.50"), 2, Cheskillsinv);
			itemset("산성구름" , Material.LINGERING_POTION, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+csd.AcidCloud.getOrDefault(p.getUniqueId(),0),"",ChatColor.UNDERLINE+"[맹독 계열]","우클릭", "활성화/비활성화 스킬", "산성 게이지 사용","",ChatColor.BOLD+""+(4+pro)+" X "+BigDecimal.valueOf(0.31*(1+csd.AcidCloud.getOrDefault(p.getUniqueId(),0)*0.04)).setScale(2, RoundingMode.HALF_EVEN)+"D","Master LV.50"), 3, Cheskillsinv);
			...
			if(Proficiency.getpro(p)<1) {
				...
				itemset("맹독", Material.ENCHANTED_BOOK, 0, 1, Arrays.asList("전체 피해량이 증가합니다","",ChatColor.GREEN+ "활성화된 물약 효과 종류수에 비례해", ChatColor.GREEN+ "받는피해가 감소합니다", "0.15 + 종류수("+ p.getActivePotionEffects().size() + ") X 0.05"), 15, Cheskillsinv);
