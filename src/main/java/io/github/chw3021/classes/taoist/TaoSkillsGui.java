	...
	public void TaoSkillsinv(Player p)
	{
		...
		if(p.getLocale().equalsIgnoreCase("ko_kr")) {
			itemset("기운", Material.CANDLE, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+tsd.Aura.getOrDefault(p.getUniqueId(), 0),"","웅크리기 + 아이템바꾸기(마우스휠)"
					,ChatColor.UNDERLINE+"✬[화염, 대지]","🖮🖰 양의기운: 파티원들의 공격력(30%), 속도 증가","",ChatColor.UNDERLINE+"✬[서리, 어둠]","🖮🖰 음의기운: ","파티원들의 방어력 30%증가","적 공격시 파티원 체력 회복", "Master Lv.1"), 0, skillsInv);
			itemset("연상", Material.BLACK_CANDLE, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+tsd.Imagery.getOrDefault(p.getUniqueId(), 0),"","손바꾸기","",ChatColor.BOLD+" X (0.7D + "+BigDecimal.valueOf(tsd.Imagery.getOrDefault(p.getUniqueId(), 0)*0.9).setScale(2, RoundingMode.HALF_EVEN)+")", "Master Lv.50"), 1, skillsInv);
			itemset("기공권", Material.YELLOW_CANDLE, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+tsd.Wave.getOrDefault(p.getUniqueId(), 0),"","우클릭","",ChatColor.BOLD+" X (0.67D + "+BigDecimal.valueOf(tsd.Wave.getOrDefault(p.getUniqueId(), 0)*0.64).setScale(2, RoundingMode.HALF_EVEN)+")" ,"Master LV.50"), 2, skillsInv);
			...
			itemset("공중제비", Material.BLUE_BANNER, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+tsd.Flip.getOrDefault(p.getUniqueId(), 0),"","점프 + 좌클릭",
					"",ChatColor.BOLD+" X (0.3D + "+BigDecimal.valueOf(tsd.Flip.getOrDefault(p.getUniqueId(), 0)*0.34).setScale(2, RoundingMode.HALF_EVEN)+")" ,"Master LV.50"), 5, skillsInv);
			itemset("잠해소비", Material.RED_CANDLE, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+tsd.CombustInside.getOrDefault(p.getUniqueId(), 0),"","웅크리기 + 근접공격",
					"",ChatColor.BOLD+" X (1.8D + "+BigDecimal.valueOf(tsd.CombustInside.getOrDefault(p.getUniqueId(), 0)*1.8).setScale(2, RoundingMode.HALF_EVEN)+")" ,"Master LV.50"), 6, skillsInv);
			itemset("내공", Material.BOOK, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+tsd.Vibe.getOrDefault(p.getUniqueId(), 0),"","공격력이 증가합니다","",ChatColor.BOLD+" + "+BigDecimal.valueOf(tsd.Vibe.getOrDefault(p.getUniqueId(), 0)*0.6).setScale(2, RoundingMode.HALF_EVEN)), 7, skillsInv);
			if(Proficiency.getpro(p)<1) {
				itemset("운기조식(잠김)", Material.STRUCTURE_VOID, 0, 1, Arrays.asList("요구 숙련도: "+ Proficiency.getproexp(p) + "/29315"), 9, skillsInv);
				...
				itemset("공명(잠김)", Material.STRUCTURE_VOID, 0, 1, Arrays.asList("요구 숙련도: "+ Proficiency.getproexp(p) + "/29315"), 15, skillsInv);
				...
				itemset("운기조식", Material.SOUL_CAMPFIRE, 0, 1, Arrays.asList("기운의 버프 효과를 두배로 증가시킵니다"), 9, skillsInv);
				itemset("기폭발", Material.FIREWORK_STAR, 0, 1, Arrays.asList("재입력시 기폭발을 사용합니다","",ChatColor.BOLD+" X (1.4D + "+BigDecimal.valueOf(tsd.Imagery.getOrDefault(p.getUniqueId(), 0)*1.5).setScale(2, RoundingMode.HALF_EVEN)+")","(피해량은 연상 레벨에 비례합니다)"), 10, skillsInv);
				itemset("기공참", Material.DIAMOND_SWORD, 0, 1, Arrays.asList("재입력시 기공참을 사용합니다","",ChatColor.BOLD+"2 X (0.68D + "+BigDecimal.valueOf(tsd.Wave.getOrDefault(p.getUniqueId(), 0)*0.76).setScale(2, RoundingMode.HALF_EVEN)+")","(피해량은 기공권 레벨에 비례합니다)"), 11, skillsInv);
				...
				itemset("속박진", Material.COARSE_DIRT, 0, 1, Arrays.asList("재입력시 속박진을 사용합니다","",ChatColor.BOLD+"10 X (0.115D + "+BigDecimal.valueOf(tsd.Charm.getOrDefault(p.getUniqueId(), 0)*0.2455).setScale(2, RoundingMode.HALF_EVEN)+")","(피해량은 부적 레벨에 비례합니다)"), 13, skillsInv);
				itemset("순보", Material.NETHERITE_BOOTS, 0, 1, Arrays.asList("재입력시 순보를 사용합니다","",ChatColor.BOLD+" X (0.9D + "+BigDecimal.valueOf(tsd.Flip.getOrDefault(p.getUniqueId(), 0)*0.9).setScale(2, RoundingMode.HALF_EVEN)+")","(피해량은 공중제비 레벨에 비례합니다)"), 14, skillsInv);
				itemset("공명", Material.BELL, 0, 1, Arrays.asList("광역피해를 적용합니다"), 15, skillsInv);
				...
				itemset("승천", Material.CUT_COPPER_SLAB, 0, 1, Arrays.asList("재입력시 승천을 사용합니다","",ChatColor.BOLD+" X (1.2D + "+BigDecimal.valueOf(tsd.Imagery.getOrDefault(p.getUniqueId(), 0)*1.3).setScale(2, RoundingMode.HALF_EVEN)+")","(피해량은 연상 레벨에 비례합니다)"), 19, skillsInv);
				itemset("기공장", Material.BOW, 0, 1, Arrays.asList("재입력시 기공장을 사용합니다","",ChatColor.BOLD+" X (0.115D + "+BigDecimal.valueOf(tsd.Charm.getOrDefault(p.getUniqueId(), 0)*0.2455).setScale(2, RoundingMode.HALF_EVEN)+")","(피해량은 기공권 레벨에 비례합니다)"), 20, skillsInv);
				itemset("만트라", Material.ACACIA_LEAVES, 0, 1, Arrays.asList("재입력시 만트라를 사용합니다","","기운 변경을 해도 5초동안 효과가 지속됩니다","",
						"5초간 파티원들의 허기를 채웁니다","",ChatColor.BOLD+" X (0.8D + "+BigDecimal.valueOf(tsd.Amplify.getOrDefault(p.getUniqueId(), 0)*0.9).setScale(2, RoundingMode.HALF_EVEN)+")","(피해량은 증폭 레벨에 비례합니다)"), 21, skillsInv);
				itemset("기공파", Material.SOUL_CAMPFIRE, 0, 1, Arrays.asList("재입력시 기공파를 사용합니다","",ChatColor.BOLD+"12 X (0.1D + "+BigDecimal.valueOf(tsd.Charm.getOrDefault(p.getUniqueId(), 0)*0.12).setScale(2, RoundingMode.HALF_EVEN)+")","(피해량은 부적 레벨에 비례합니다)"), 22, skillsInv);
			...
			itemset("Aura", Material.CANDLE, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+tsd.Aura.getOrDefault(p.getUniqueId(), 0),"","Sneaking + Item Change(MouseWheel)"
					,ChatColor.UNDERLINE+"✬[Fire, Earth]","🖮🖰 Aura Of Positive: Increases Party Damage(30%), Speed","",ChatColor.UNDERLINE+"✬[Frost, Dark]","🖮🖰 Aura Of Negative: ","Increases Party Armor 30%","Heal Party When You Attack", "Master Lv.1"), 0, skillsInv);
			itemset("Imagery", Material.BLACK_CANDLE, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+tsd.Imagery.getOrDefault(p.getUniqueId(), 0),"","SwapHand","",ChatColor.BOLD+" X (0.7D + "+BigDecimal.valueOf(tsd.Imagery.getOrDefault(p.getUniqueId(), 0)*0.9).setScale(2, RoundingMode.HALF_EVEN)+")", "Master Lv.50"), 1, skillsInv);
			itemset("Wave", Material.YELLOW_CANDLE, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+tsd.Wave.getOrDefault(p.getUniqueId(), 0),"","Rightclick","",ChatColor.BOLD+" X (0.67D + "+BigDecimal.valueOf(tsd.Wave.getOrDefault(p.getUniqueId(), 0)*0.64).setScale(2, RoundingMode.HALF_EVEN)+")" ,"Master LV.50"), 2, skillsInv);
			itemset("Amplify", Material.BROWN_CANDLE, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+tsd.Amplify.getOrDefault(p.getUniqueId(), 0),"","Sneaking + Rightclick","Increases Aura Range For" + BigDecimal.valueOf(tsd.Amplify.getOrDefault(p.getUniqueId(), 0)*0.1).setScale(2, RoundingMode.HALF_EVEN) + "secs",
					"Aura Of Positive: Attack Near By Entity",ChatColor.BOLD+" X (0.2D + "+BigDecimal.valueOf(tsd.Amplify.getOrDefault(p.getUniqueId(), 0)*0.2).setScale(2, RoundingMode.HALF_EVEN)+")","","Aura Of Negetive: Heal Party Instantly" ,"Master LV.50"), 3, skillsInv);
			...
			itemset("CombustInside", Material.RED_CANDLE, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+tsd.CombustInside.getOrDefault(p.getUniqueId(), 0),"","Sneaking + Hit",
					"",ChatColor.BOLD+" X (1.8D + "+BigDecimal.valueOf(tsd.CombustInside.getOrDefault(p.getUniqueId(), 0)*1.8).setScale(2, RoundingMode.HALF_EVEN)+")" ,"Master LV.50"), 6, skillsInv);
			itemset("Vibe", Material.BOOK, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+tsd.Vibe.getOrDefault(p.getUniqueId(), 0),"","Increases damage","",ChatColor.BOLD+" + "+BigDecimal.valueOf(tsd.Vibe.getOrDefault(p.getUniqueId(), 0)*0.6).setScale(2, RoundingMode.HALF_EVEN)), 7, skillsInv);
			if(Proficiency.getpro(p)<1) {
				itemset("MentalTraining(Locked)", Material.STRUCTURE_VOID, 0, 1, Arrays.asList("Required Proficiency: "+ Proficiency.getproexp(p) + "/29315"), 9, skillsInv);
				...
				itemset("Resonance(Locked)", Material.STRUCTURE_VOID, 0, 1, Arrays.asList("Required Proficiency: "+ Proficiency.getproexp(p) + "/29315"), 15, skillsInv);
				...
				itemset("MentalTraining", Material.SOUL_CAMPFIRE, 0, 1, Arrays.asList("Doubles Aura Buff Effect"), 9, skillsInv);
				itemset("Blast", Material.FIREWORK_STAR, 0, 1, Arrays.asList("Use Blast When Use Once More","",ChatColor.BOLD+" X (1.4D + "+BigDecimal.valueOf(tsd.Imagery.getOrDefault(p.getUniqueId(), 0)*1.5).setScale(2, RoundingMode.HALF_EVEN)+")","(Damage Affected By Imagery)"), 10, skillsInv);
				itemset("CharmSlash", Material.DIAMOND_SWORD, 0, 1, Arrays.asList("Use CharmSlash When Use Once More","",ChatColor.BOLD+"2 X (0.68D + "+BigDecimal.valueOf(tsd.Wave.getOrDefault(p.getUniqueId(), 0)*0.76).setScale(2, RoundingMode.HALF_EVEN)+")","(Damage Affected By Wave)"), 11, skillsInv);
				...
				itemset("BindingFormation", Material.COARSE_DIRT, 0, 1, Arrays.asList("Use BindingFormation When Use Once More","",ChatColor.BOLD+"10 X (0.115D + "+BigDecimal.valueOf(tsd.Charm.getOrDefault(p.getUniqueId(), 0)*0.2455).setScale(2, RoundingMode.HALF_EVEN)+")","(Damage Affected By Charm)"), 13, skillsInv);
				itemset("Shunpo", Material.NETHERITE_BOOTS, 0, 1, Arrays.asList("Use Shunpo When Use Once More","",ChatColor.BOLD+" X (0.9D + "+BigDecimal.valueOf(tsd.Flip.getOrDefault(p.getUniqueId(), 0)*0.9).setScale(2, RoundingMode.HALF_EVEN)+")","(Damage Affected By Flip)"), 14, skillsInv);
				itemset("Resonance", Material.BELL, 0, 1, Arrays.asList("Inflict splash damage"), 15, skillsInv);
				...
				itemset("Ascension", Material.CUT_COPPER_SLAB, 0, 1, Arrays.asList("Use Ascension When Use Once More","",ChatColor.BOLD+" X (1.2D + "+BigDecimal.valueOf(tsd.Imagery.getOrDefault(p.getUniqueId(), 0)*1.3).setScale(2, RoundingMode.HALF_EVEN)+")","(Damage Affected By Imagery)"), 19, skillsInv);
				itemset("SpiritStorm", Material.BOW, 0, 1, Arrays.asList("Use SpiritStorm When Use Once More","",ChatColor.BOLD+" X (0.115D + "+BigDecimal.valueOf(tsd.Charm.getOrDefault(p.getUniqueId(), 0)*0.2455).setScale(2, RoundingMode.HALF_EVEN)+")","(Damage Affected By Wave)"), 20, skillsInv);
				itemset("Mantra", Material.ACACIA_LEAVES, 0, 1, Arrays.asList("Use Mantra When Use Once More","","Aura Effects Remain For Amplify Duration","Even if you Change Aura","",
						"Give Saturation Effect to Party","For 5 seconds","",ChatColor.BOLD+" X (0.8D + "+BigDecimal.valueOf(tsd.Amplify.getOrDefault(p.getUniqueId(), 0)*0.9).setScale(2, RoundingMode.HALF_EVEN)+")","(Damage Affected By Amplify)"), 21, skillsInv);
				itemset("EnergyBlast", Material.SOUL_CAMPFIRE, 0, 1, Arrays.asList("Use EnergyBlast When Use Once More","",ChatColor.BOLD+"12 X (0.1D + "+BigDecimal.valueOf(tsd.Charm.getOrDefault(p.getUniqueId(), 0)*0.12).setScale(2, RoundingMode.HALF_EVEN)+")","(Damage Affected By Charm)"), 22, skillsInv);
