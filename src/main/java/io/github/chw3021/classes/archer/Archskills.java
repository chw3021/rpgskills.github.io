	...
	final private void SpreadingArrows(Player p) {
		...
		@Override
            ...
            {	
        		...
        		if(p.getLocale().equalsIgnoreCase("ko_kr")) {
    				if(Proficiency.getpro(p) >=1) {
    					p.spigot().sendMessage(ChatMessageType.ACTION_BAR, new ComponentBuilder(ChatColor.BLUE+"È­»ì»Ñ¸®±â "+sa.get(p.getName()) + "/6").create());
    				}
    				else {
    					p.spigot().sendMessage(ChatMessageType.ACTION_BAR, new ComponentBuilder(ChatColor.BLUE+"È­»ì»Ñ¸®±â "+sa.get(p.getName()) + "/3").create());
    				}
			    }
        		else {
    				if(Proficiency.getpro(p) >=1) {
				            		...
				            		if(Proficiency.getpro(p) >=1) {
					                	if(sa.get(p.getName())<6) {
						                	...
						                	if(sa.get(p.getName())<5) {
							            		Bukkit.getPluginManager().callEvent(new SkillUseEvent(p,5.0,0,ChatColor.BLUE+"È­»ì»Ñ¸®±â",ChatColor.BLUE+"SpreadingArrows"));
						                	}
					                	}
				            		}
				            		else {
	...
	public void Indure(EntityDamageEvent d) 
	{
		if(d.getEntity() instanceof Player) 
		{
			...
			if(ClassData.pc.get(p.getUniqueId()) == 6 && indure.containsKey(p)) {
        		...
        		if(p.getLocale().equalsIgnoreCase("ko_kr")) {
    				p.spigot().sendMessage(ChatMessageType.ACTION_BAR, new ComponentBuilder(ChatColor.BLUE+"[¹öÆ¼±â]").create());
			    }
        		else {
