...
public class ArchSkillsGui extends SkillsGui{
	...
	public void Archskillsinv(Player p)
	{
		...
		if(p.getLocale().equalsIgnoreCase("ko_kr")) {
			...
			itemset("회수", Material.TIPPED_ARROW, 2, 1, Arrays.asList(ChatColor.AQUA+"LV."+asd.Retrieve.getOrDefault(p.getUniqueId(),0),"",ChatColor.UNDERLINE+"[대지 계열]","손바꾸기","",ChatColor.BOLD+"X 0.5 X (1 + 0.23D X 적에게 박힌 화살수)", "Master LV.1"), 1, Archskillsinv);
			itemset("속사", Material.TIPPED_ARROW, 3, 1, Arrays.asList(ChatColor.AQUA+"LV."+asd.RapidFire.getOrDefault(p.getUniqueId(),0),"",ChatColor.UNDERLINE+"[대지 계열]","웅크리기 + 손바꾸기","",ChatColor.BOLD+"15 X "+BigDecimal.valueOf(0.034*(1+asd.RapidFire.getOrDefault(p.getUniqueId(),0)*0.01)).setScale(3, RoundingMode.HALF_EVEN)+"D", "Master LV.50"), 2, Archskillsinv);
			...
			itemset("궁술", Material.BOOK, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+asd.Archery.getOrDefault(p.getUniqueId(),0),"","공격력이 증가합니다",ChatColor.BOLD+"X "+BigDecimal.valueOf(1+asd.Archery.getOrDefault(p.getUniqueId(),0)*0.035).setScale(2, RoundingMode.HALF_EVEN),"","활시위를 당길때 1초동안 피해를 버팁니다",String.format(ChatColor.BOLD+"-%.2f", BigDecimal.valueOf(1-0.68 + Proficiency.getpro(p)*0.2)) ), 7, Archskillsinv);
			if(Proficiency.getpro(p)<1) {
				...
				itemset("나선화살(잠김)", Material.STRUCTURE_VOID, 0, 1, Arrays.asList("요구 숙련도: "+ Proficiency.getproexp(p) + "/29315"), 11, Archskillsinv);
				...
				itemset("나선화살", Material.SPECTRAL_ARROW, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"[대지 계열]","재입력시 나선화살을 쏩니다", "(피해량은 속사 레벨에 비례합니다)","",ChatColor.BOLD+"10 X "+demical(0.34*(1+asd.RapidFire.get(p.getUniqueId())*0.2))+"D"), 11, Archskillsinv);
				...
				itemset("곡사(잠김)", Material.STRUCTURE_VOID, 0, 1, Arrays.asList("요구 숙련도: "+ Proficiency.getproexp(p) + "/155015"), 20, Archskillsinv);
				...
				itemset("곡사", Material.SPECTRAL_ARROW, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"[대지 계열]","재입력시 곡사을 쏩니다", "(피해량은 속사 레벨에 비례합니다)","",ChatColor.BOLD+"20 X "+demical(0.28*(1+asd.RapidFire.get(p.getUniqueId())*0.16))+"D"), 20, Archskillsinv);
			...
			itemset("Retrieve", Material.TIPPED_ARROW, 2, 1, Arrays.asList(ChatColor.AQUA+"LV."+asd.Retrieve.getOrDefault(p.getUniqueId(),0),"",ChatColor.UNDERLINE+"[Earth]","SwapHand","",ChatColor.BOLD+" 0.5 X X (1 + 0.23D X number of arrows in enemy)", "Master LV.1"), 1, Archskillsinv);
			...
			if(Proficiency.getpro(p)<1) {
				...
				itemset("SpinShots(Locked)", Material.STRUCTURE_VOID, 0, 1, Arrays.asList("Required Proficiency: "+ Proficiency.getproexp(p) + "/29315"), 11, Archskillsinv);
				itemset("ScatterShot(Locked)", Material.STRUCTURE_VOID, 0, 1, Arrays.asList("Required Proficiency: "+ Proficiency.getproexp(p) + "/29315"), 12, Archskillsinv);
				...
				itemset("SpinShots", Material.SPECTRAL_ARROW, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"[Earth]","Use SpinShots When Use Once More","(Damage Affected By RapidFire)","",ChatColor.BOLD+"10 X "+demical(0.34*(1+asd.RapidFire.get(p.getUniqueId())*0.2))+"D"), 11, Archskillsinv);
				itemset("ScatterShot", Material.TARGET, 0, 1, Arrays.asList("Shoot Three Times"), 12, Archskillsinv);
				...
				itemset("ArcShot(Locked)", Material.STRUCTURE_VOID, 0, 1, Arrays.asList("Required Proficiency: "+ Proficiency.getproexp(p) + "/155015"), 20, Archskillsinv);
				...
				itemset("ArcShot", Material.SPECTRAL_ARROW, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"[Earth]","Use ArcShot When Use Once More","(Damage Affected By RapidFire)","",ChatColor.BOLD+"20 X "+demical(0.28*(1+asd.RapidFire.get(p.getUniqueId())*0.16))+"D"), 20, Archskillsinv);
