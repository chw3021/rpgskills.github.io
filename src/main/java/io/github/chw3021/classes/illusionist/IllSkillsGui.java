...
public class IllSkillsGui{
	...
	public void IllSkillsinv(Player p)
	{
		...
		if(p.getLocale().equalsIgnoreCase("ko_kr")) {
			...
			itemset("허수아비", Material.ARMOR_STAND, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+isd.FakeDoll.getOrDefault(p.getUniqueId(),0),"","손바꾸기 + 웅크리기", "주변 적들을 도발합니다", "Master LV.1"), 5, Illskillsinv);
			...
			if(Proficiency.getpro(p)<1) {
				...
				itemset("위치변경", Material.SKELETON_SKULL, 0, 1, Arrays.asList("재입력시 허수아비와 자신의 위치를 바꿉니다"), 14, Illskillsinv);
				itemset("조작", Material.ENCHANTED_BOOK, 0, 1, Arrays.asList("공격력이 증가합니다"), 16, Illskillsinv);
				itemset("공허의 환상", Material.WRITTEN_BOOK, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"[바람 계열]","웅크리기 + 아이템던지기","",ChatColor.BOLD+" X 15.6D"), 17, Illskillsinv);
				...
				itemset("조작된차원(잠김)", Material.STRUCTURE_VOID, 0, 1, Arrays.asList("요구 숙련도: "+ Proficiency.getproexp(p) + "/155015"), 26, Illskillsinv);
			}
			else {
				itemset("혼란", Material.END_PORTAL_FRAME, 0, 1, Arrays.asList("적을 짧게 제압합니다"), 9, Illskillsinv);
				itemset("앙코르", Material.CHORUS_FLOWER, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"[바람 계열]","재입력시 앙코르를 사용합니다","(피해량은 속임수 레벨에 비례합니다)","",ChatColor.BOLD+" X "+BigDecimal.valueOf(0.8*(1+isd.Trick.getOrDefault(p.getUniqueId(),0)*0.0535)).setScale(2, RoundingMode.HALF_EVEN)+"D"), 10, Illskillsinv);
				...
				itemset("최면", Material.CANDLE, 0, 1, Arrays.asList("허수아비 주변의 적들을 재웁니다","잠든 적은 공격을 받으면","더 많은 피해를 입고 깨어납니다"), 22, Illskillsinv);
				...
				itemset("조작된차원", Material.WRITTEN_BOOK, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"[바람 계열]","달리기 + 아이템던지기", "조작된 차원에 가면 인형하나가 있습니다", "인형은 스킬을 통해 피해를 입힐수있습니다", "6초후 복귀와 동시에 차원으로 이동하기전","탐색되었던 주변 적들을 공격합니다", "", "공격력은 인형에게 입혔던 피해량에 비례합니다",ChatColor.BOLD+"(인형에게 입힌 피해량) X 0.005D"), 26, Illskillsinv);
			}
