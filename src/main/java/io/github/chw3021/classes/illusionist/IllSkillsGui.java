	...
	public void IllSkillsinv(Player p)
	{
		...
		if(p.getLocale().equalsIgnoreCase("ko_kr")) {
			...
			if(Proficiency.getpro(p)<1) {
				...
				itemset("피날레", Material.WRITTEN_BOOK, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"[바람 계열]","달리기 + 아이템던지기", "역설의 대기시간이 80% 감소합니다", "기술사용시 피날레 효과가 중첩됩니다","8중첩 달성시 추가 고정피해를 입힙니다",ChatColor.BOLD+"피해량 X (4+0.01XD)%","10초동안 지속됩니다"), 26, Illskillsinv);
			}
