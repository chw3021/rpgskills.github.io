...
public class IllSkillsGui extends SkillsGui{
	...
	public void IllSkillsinv(Player p)
	{
		...
		if(p.getLocale().equalsIgnoreCase("ko_kr")) {
			...
			itemset("왜곡", Material.DEBUG_STICK, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+isd.Distortion.getOrDefault(p.getUniqueId(),0),"",ChatColor.UNDERLINE+"✬[바람]","🖮🖰 우클릭","",ChatColor.BOLD+" X "+BigDecimal.valueOf(1.626*(1+isd.Distortion.getOrDefault(p.getUniqueId(),0)*0.0835)).setScale(2, RoundingMode.HALF_EVEN)+"D", "Master LV.50"), 3, skillsInv);
			itemset("역설", Material.DEBUG_STICK, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+isd.Paradox.getOrDefault(p.getUniqueId(),0),"","웅크리기 + 우클릭","궁극기를 제외한 모든 기술의","재사용 대기시간을 초기화 합니다","해로운 효과를 모두 제거합니다","체력과 허기를 모두 회복합니다", "Master LV.1"), 4, skillsInv);
			itemset("허수아비", Material.ARMOR_STAND, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+isd.FakeDoll.getOrDefault(p.getUniqueId(),0),"","손바꾸기 + 웅크리기", "주변 적들을 도발합니다", "0.5초동안 무적상태가 됩니다", "Master LV.1"), 5, skillsInv);
			...
			itemset("Paradox", Material.DEBUG_STICK, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+isd.Paradox.getOrDefault(p.getUniqueId(),0),"","Sneaking + Rightclick","Reset All CoolDown","Remove negetive Effect","Reset HP And Hunger", "Master LV.1"), 4, skillsInv);
			itemset("FakeDoll", Material.ARMOR_STAND, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+isd.FakeDoll.getOrDefault(p.getUniqueId(),0),"","SwapHand + Sneaking", "Taunts shortly near by enemies", "Get invincible state for 0.5s","", "Master LV.1"), 5, skillsInv);
			...
			if(Proficiency.getpro(p)<1) {
				...
				itemset("Change", Material.SKELETON_SKULL, 0, 1, Arrays.asList("Switchs Pos with FakeDoll When use Once More", "Get invincible state for 0.5s"), 14, skillsInv);
