	...
	public void Berskillsinv(Player p) {
		...
		if (p.getLocale().equalsIgnoreCase("ko_kr")) {
			...
			basic("변신", Material.WOODEN_HOE,0, 1, Arrays.asList(	"손바꾸기", "대기시간 0.5초", "",	ChatColor.DARK_RED + "갈망: 받는피해가 20% 감소하고", ChatColor.DARK_RED + "적을 공격시 체력을 회복합니다",
					...
					0,0d,0d,1, 0, skillsInv);
			basic("분사", Material.STONE_SWORD, 0, 1,Arrays.asList(ChatColor.UNDERLINE+"✬[대지]", "웅크리기 + 손바꾸기"),bsd.Spray.getOrDefault(p.getUniqueId(), 0),1,0.8 , 0.07, 50,1, skillsInv);
			basic("불사", Material.APPLE, 0, 1, Arrays.asList("체력이 5%미만으로 떨어질시", "불사가 발동됩니다", "패시브 스킬"), 2, skillsInv);
			basic("휩쓸기", Material.IRON_HOE, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"✬[대지]", "좌클릭 + 웅크리기" ),bsd.Swipe.getOrDefault(p.getUniqueId(), 0),1,0.5,0.08,50, 3, skillsInv);
			basic("흡입", Material.GOLDEN_HOE, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"✬[대지]", "점프 + 우클릭"),bsd.Inhale.getOrDefault(p.getUniqueId(), 0),8,0.2,0.02,50, 4, skillsInv);
			basic("난무", Material.DIAMOND_SWORD, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"✬[대지]", "점프 + 좌클릭"),bsd.Flurry.getOrDefault(p.getUniqueId(), 0),
					10,0.18,0.02,50,5, skillsInv);
			...
			if (Proficiency.getpro(p) < 1) {
				...
				itemset("갈증", Material.CRIMSON_HYPHAE, 0, 1, Arrays.asList("갈망 상태중 포만감 효과도 얻습니다"), 9, skillsInv);
				...
				itemset("들끓는 심장", Material.WRITTEN_BOOK, 0, 1,
						Arrays.asList(ChatColor.UNDERLINE+"✬[화염]", "웅크리기 + 아이템던지기", "15초동안 공격력이 대폭 증가합니다",
				...
				itemset("말살", Material.WRITTEN_BOOK, 0, 1,
						Arrays.asList(ChatColor.UNDERLINE+"✬[대지]", "달리기 + 아이템던지기",
			...
			if (Proficiency.getpro(p) < 1) {
				...
				itemset("Scratch", Material.NETHER_WART, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"✬[Earth]", "Scratch Enemies When use Once more",
						"(Damage Affected By Swipe)"),bsd.Swipe.getOrDefault(p.getUniqueId(), 0),1,0.7,0.09, 12, skillsInv);
				...
				itemset("Smite", Material.CRIMSON_PRESSURE_PLATE, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"✬[Earth]",
						"Smite When use Once more", "(Damage Affected By CrimsonAdvance)"),bsd.Crimpson.getOrDefault(p.getUniqueId(), 0),1,1.36,0.147, 15, skillsInv);
