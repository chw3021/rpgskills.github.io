	...
	public void OceSkillsinv(Player p)
	{
		...
		if(p.getLocale().equalsIgnoreCase("ko_kr")) {
			...
			if(Proficiency.getpro(p)<1) {
				...
				itemset("삼지창폭발", Material.DARK_PRISMARINE, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"❈ [물]","🖮🖰 꿰뚫기이후 재공격시 삼지창폭발을 사용합니다","(피해량은 삼지창돌격 레벨에 비례합니다)","",ChatColor.BOLD+" X "+BigDecimal.valueOf(0.9*(1+fsd.OceanCharge.getOrDefault(p.getUniqueId(),0)*0.065)).setScale(2, RoundingMode.HALF_EVEN)+"D"), 13, skillsInv);
			...
			if(Proficiency.getpro(p)<1) {
				...
				itemset("TridentExplosion", Material.DARK_PRISMARINE, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"❈ [Water]","🖮🖰 Use TridentExplosion When Use Once More","(Damage Affected By OceanCharge)","",ChatColor.BOLD+" X "+BigDecimal.valueOf(0.9*(1+fsd.OceanCharge.getOrDefault(p.getUniqueId(),0)*0.065)).setScale(2, RoundingMode.HALF_EVEN)+"D"), 13, skillsInv);
