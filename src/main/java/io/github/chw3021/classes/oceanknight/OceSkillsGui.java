...
public class OceSkillsGui extends SkillsGui{
	...
	public void OceSkillsinv(Player p)
	{
		...
		if(p.getLocale().equalsIgnoreCase("ko_kr")) {
			...
			itemset("삼지창돌격", Material.TRIDENT, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+fsd.OceanCharge.getOrDefault(p.getUniqueId(),0),"",ChatColor.UNDERLINE+"[물 계열]","웅크리기 + 좌클릭","",ChatColor.BOLD+" X "+BigDecimal.valueOf(0.6*(1+fsd.OceanCharge.getOrDefault(p.getUniqueId(),0)*0.055)).setScale(2, RoundingMode.HALF_EVEN)+"D", "Master Lv.50"), 4, Oceskillsinv);
