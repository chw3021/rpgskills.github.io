	...
	public void WdcSkillsinv(Player p)
	{
		...
		if(p.getLocale().equalsIgnoreCase("ko_kr")) {
			...
			if(Proficiency.getpro(p)<1) {
				...
				itemset("팬텀슴격(잠김)", Material.STRUCTURE_VOID, 0, 1, Arrays.asList("요구 숙련도: "+ Proficiency.getproexp(p) + "/29315"), 12, Wdcskillsinv);
				itemset("가벼운발걸음(잠김)", Material.STRUCTURE_VOID, 0, 1, Arrays.asList("요구 숙련도: "+ Proficiency.getproexp(p) + "/29315"), 13, Wdcskillsinv);
				itemset("풍요의토템(잠김)", Material.STRUCTURE_VOID, 0, 1, Arrays.asList("요구 숙련도: "+ Proficiency.getproexp(p) + "/29315"), 14, Wdcskillsinv);
				...
				itemset("팬텀슴격", Material.BAT_SPAWN_EGG, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"[맹독 계열]","재입력시 팬텀슴격을 사용합니다","(피해량은 망령 레벨에 비레합니다)","",ChatColor.BOLD+"19 X (0.3D + "+BigDecimal.valueOf(wsd.Wraith.getOrDefault(p.getUniqueId(), 0)*0.342).setScale(2, RoundingMode.HALF_EVEN)+"), 0.5D + "+BigDecimal.valueOf(wsd.Wraith.getOrDefault(p.getUniqueId(), 0)*0.442).setScale(2, RoundingMode.HALF_EVEN)), 12, Wdcskillsinv);
				itemset("가벼운발걸음", Material.CHAINMAIL_BOOTS, 0, 1, Arrays.asList("유체이탈 종료후 이동속도와 점프 효과를 얻습니다"), 13, Wdcskillsinv);
				itemset("풍요의토템", Material.TOTEM_OF_UNDYING, 0, 1, Arrays.asList("파티원들의 허기를 채웁니다"), 14, Wdcskillsinv);
				...
				itemset("바롱사메디", Material.WRITTEN_BOOK, 0, 1, Arrays.asList("웅크리기 + num4","파티원들이 죽는것을 방지해줍니다", "파티원들의 공격력이 증가합니다","",ChatColor.BOLD+"+ "+BigDecimal.valueOf(p.getLevel()*0.5).setScale(2, RoundingMode.HALF_EVEN)), 17, Wdcskillsinv);
				...
				itemset("용기의토템(잠김)", Material.STRUCTURE_VOID, 0, 1, Arrays.asList("요구 숙련도: "+ Proficiency.getproexp(p) + "/155015"), 23, Wdcskillsinv);
				...
				itemset("희생", Material.ZOMBIE_HEAD, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"[맹독 계열]","재입력시 좀비들을 폭발시킵니다","","만약 좀비가 없다면","6마리를 즉시 소환합니다","","죽음의 기사는 사라지지 않고","두배의 피해량을 줍니다","(피해량은 망령 레벨에 비레합니다)","",ChatColor.BOLD+" X (1.8D + "+BigDecimal.valueOf(wsd.Wraith.getOrDefault(p.getUniqueId(), 0)*2.5).setScale(2, RoundingMode.HALF_EVEN)+")"), 21, Wdcskillsinv);
				itemset("용기의토템", Material.TOTEM_OF_UNDYING, 0, 1, Arrays.asList("파티원들의 해로운효과를 제거합니다"), 23, Wdcskillsinv);
				...
				itemset("영원의 마법진", Material.WRITTEN_BOOK, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"[맹독 계열]","웅크리기 + num5","파티에게 힘,재생 효과를 제공합니다","적에게 약함,둔화 효과를 줍니다","좀비대신 죽음의 기사가 소환됩니다","마법진은 15초뒤에 폭발합니다","폭발전 마법진 위에 있던 적들에게","자신과 파티원이 입힌 피해량에 비례해"
			...
			if(Proficiency.getpro(p)<1) {
				...
				itemset("TotemOfAbundant(Locked)", Material.STRUCTURE_VOID, 0, 1, Arrays.asList("Required Proficiency: "+ Proficiency.getproexp(p) + "/29315"), 14, Wdcskillsinv);
				...
				itemset("PhantomSwoop", Material.BAT_SPAWN_EGG, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"[Poison]","Use PhantomSwoop When Use Once More","(Damage Affected By Wraith)","",ChatColor.BOLD+"19 X (0.3D + "+BigDecimal.valueOf(wsd.Wraith.getOrDefault(p.getUniqueId(), 0)*0.342).setScale(2, RoundingMode.HALF_EVEN)+"), 0.5D + "+BigDecimal.valueOf(wsd.Wraith.getOrDefault(p.getUniqueId(), 0)*0.442).setScale(2, RoundingMode.HALF_EVEN)), 12, Wdcskillsinv);
				...
				itemset("TotemOfAbundant", Material.TOTEM_OF_UNDYING, 0, 1, Arrays.asList("Totem Will Saturatify Near By Party"), 14, Wdcskillsinv);
				...
				itemset("Baron Samedi", Material.WRITTEN_BOOK, 0, 1, Arrays.asList("Sneaking + num4","Prevent Party From Death", "Increases Party Damage","",ChatColor.BOLD+"+ "+BigDecimal.valueOf(p.getLevel()*0.5).setScale(2, RoundingMode.HALF_EVEN)), 17, Wdcskillsinv);
				...
				itemset("TotemOfBravery(Locked)", Material.STRUCTURE_VOID, 0, 1, Arrays.asList("Required Proficiency: "+ Proficiency.getproexp(p) + "/155015"), 23, Wdcskillsinv);
				...
				itemset("Sacrifice", Material.ZOMBIE_HEAD, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"[Poison]","Explode Remaining Zombies","When Use Once More","","If There are No zombies","Summon 6 Zombies Instantly","","Death Knight won't disappear","And damage double","(Damage Affected By Wraith)","",ChatColor.BOLD+" X (1.8D + "+BigDecimal.valueOf(wsd.Wraith.getOrDefault(p.getUniqueId(), 0)*2.5).setScale(2, RoundingMode.HALF_EVEN)+")"), 21, Wdcskillsinv);
				itemset("TotemOfBravery", Material.TOTEM_OF_UNDYING, 0, 1, Arrays.asList("Removes negetive effects of party"), 23, Wdcskillsinv);
