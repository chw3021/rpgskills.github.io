...
public class WdcSkillsGui{
	...
	public void WdcSkillsinv(Player p)
	{
		...
		if(p.getLocale().equalsIgnoreCase("ko_kr")) {
			itemset("독사의송곳니", Material.STONE_HOE, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+wsd.Fangs.getOrDefault(p.getUniqueId(), 0),"",ChatColor.UNDERLINE+"[맹독 계열]","웅크리기 + 손바꾸기", "괭이 사용중 시전","",ChatColor.BOLD+"5 X (0.4D + "+BigDecimal.valueOf(wsd.Fangs.getOrDefault(p.getUniqueId(), 0)*0.3).setScale(2, RoundingMode.HALF_EVEN)+")", "Master LV.50"), 0, Wdcskillsinv);
			itemset("수호의로아:보수", Material.TOTEM_OF_UNDYING, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+wsd.Bosou.getOrDefault(p.getUniqueId(), 0),"","웅크리기 + 손바꾸기", "토템 사용중 시전", 
			...
			itemset("수확", Material.IRON_HOE, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+wsd.Harvest.getOrDefault(p.getUniqueId(), 0),"",ChatColor.UNDERLINE+"[대지 계열]","우클릭", "괭이 사용중 시전", "영혼이 아직 흡수되지 않은 적에게 재사용시", "체력과 허기를 즉시 회복합니다", "적이 받는 피해를 15%증가시킵니다","",ChatColor.BOLD+" X (0.5D + "+BigDecimal.valueOf(wsd.Harvest.getOrDefault(p.getUniqueId(), 0)*0.56).setScale(2, RoundingMode.HALF_EVEN)+")", "Master LV.50"), 2, Wdcskillsinv);
			itemset("망령", Material.GOLDEN_HOE, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+wsd.Wraith.getOrDefault(p.getUniqueId(), 0),"",ChatColor.UNDERLINE+"[화염 계열]","우클릭 + 웅크리기", "괭이 사용중 시전","",ChatColor.BOLD+"20 X (0.2D + "+BigDecimal.valueOf(wsd.Wraith.getOrDefault(p.getUniqueId(), 0)*0.2).setScale(2, RoundingMode.HALF_EVEN)+")", "Master LV.50"), 3, Wdcskillsinv);
			itemset("유체이탈", Material.TOTEM_OF_UNDYING, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+wsd.AstralProjection.getOrDefault(p.getUniqueId(), 0),"","우클릭", "토템 사용중 시전", "웅크리기시 종료", "Master LV.1"), 4, Wdcskillsinv);
			itemset("주술", Material.TOTEM_OF_UNDYING, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+wsd.Incantation.getOrDefault(p.getUniqueId(), 0),"","우클릭 + 웅크리기", "토템 사용중 시전", "Master LV.1"), 5, Wdcskillsinv);
			...
			if(Proficiency.getpro(p)<1) {
				...
				itemset("조글린돌격", Material.ZOGLIN_SPAWN_EGG, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"[맹독 계열]","재입력시 조글린돌격을 사용합니다","(피해량은 독사의송곳니 레벨에 비레합니다)","",ChatColor.BOLD+" X (0.7D + "+BigDecimal.valueOf(wsd.Fangs.getOrDefault(p.getUniqueId(), 0)*0.8).setScale(2, RoundingMode.HALF_EVEN)+")"), 9, Wdcskillsinv);
				...
				itemset("팬텀의슴격", Material.BAT_SPAWN_EGG, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"[맹독 계열]","재입력시 팬텀의슴격을 사용합니다","(피해량은 망령 레벨에 비레합니다)","",ChatColor.BOLD+"20 X (0.2D + "+BigDecimal.valueOf(wsd.Wraith.getOrDefault(p.getUniqueId(), 0)*0.2).setScale(2, RoundingMode.HALF_EVEN)+")"), 12, Wdcskillsinv);
				...
				itemset("죽음의표식(잠김)", Material.STRUCTURE_VOID, 0, 1, Arrays.asList("요구 숙련도: "+ Proficiency.getproexp(p) + "/155015"), 26, Wdcskillsinv);
			}
			else {
				itemset("조글린돌격", Material.ZOGLIN_SPAWN_EGG, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"[맹독 계열]","재입력시 조글린돌격을 사용합니다","(피해량은 독사의송곳니 레벨에 비레합니다)","",ChatColor.BOLD+" X (0.7D + "+BigDecimal.valueOf(wsd.Fangs.getOrDefault(p.getUniqueId(), 0)*0.8).setScale(2, RoundingMode.HALF_EVEN)+")"), 9, Wdcskillsinv);
				itemset("금술", Material.NETHER_BRICK, 0, 1, Arrays.asList("재입력시 금술을 사용합니다","파티원 각각의 체력을 20% 소모합니다", "재생효과를 주고 "+ChatColor.BOLD+BigDecimal.valueOf(wsd.Bosou.getOrDefault(p.getUniqueId() ,0)*0.5).setScale(2, RoundingMode.HALF_EVEN)+ "초동안 무적상태로 만듭니다") , 10, Wdcskillsinv);
				...
				itemset("죽음의표식", Material.WRITTEN_BOOK, 0, 1, Arrays.asList("달리기 + 아이템던지기","주변 적들을 제압하고 표식을 남깁니다","적들은 4초동안 데미지를 받지않고 죽지않습니다","","4초뒤 적들은 (이전 4초동안 받은 피해량)"+ 
						ChatColor.BOLD+" X "+BigDecimal.valueOf((1+ p.getLevel()*0.001)).setScale(4, RoundingMode.HALF_EVEN)+"의 피해를 입습니다"), 26, Wdcskillsinv);
			}
			...
			itemset("Fangs", Material.STONE_HOE, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+wsd.Fangs.getOrDefault(p.getUniqueId(), 0),"",ChatColor.UNDERLINE+"[Poison]","Sneaking + SwapHand", "Holding Hoe","",ChatColor.BOLD+"5 X (0.4D + "+BigDecimal.valueOf(wsd.Fangs.getOrDefault(p.getUniqueId(), 0)*0.3).setScale(2, RoundingMode.HALF_EVEN)+")", "Master LV.50"), 0, Wdcskillsinv);
			itemset("Bosou", Material.TOTEM_OF_UNDYING, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+wsd.Bosou.getOrDefault(p.getUniqueId(), 0),"","Sneaking + SwapHand", "Holding Totem", 
			...
			itemset("Harvest", Material.IRON_HOE, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+wsd.Harvest.getOrDefault(p.getUniqueId(), 0),"",ChatColor.UNDERLINE+"[Earth]","RightClick", "Holding Hoe", "If you reuse To Enemy", "Whose soul has not yet been absorbed", "you will immediately recover hp and hunger", "Increases Damage Enemy take 15%","",ChatColor.BOLD+" X (0.5D + "+BigDecimal.valueOf(wsd.Harvest.getOrDefault(p.getUniqueId(), 0)*0.56).setScale(2, RoundingMode.HALF_EVEN)+")", "Master LV.50"), 2, Wdcskillsinv);
			itemset("Wraith", Material.GOLDEN_HOE, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+wsd.Wraith.getOrDefault(p.getUniqueId(), 0),"",ChatColor.UNDERLINE+"[Flame]","RightClick + Sneaking", "Holding Hoe","",ChatColor.BOLD+"20 X (0.2D + "+BigDecimal.valueOf(wsd.Wraith.getOrDefault(p.getUniqueId(), 0)*0.2).setScale(2, RoundingMode.HALF_EVEN)+")", "Master LV.50"), 3, Wdcskillsinv);
			itemset("AstralProjection", Material.TOTEM_OF_UNDYING, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+wsd.AstralProjection.getOrDefault(p.getUniqueId(), 0),"","RightClick", "Holding Totem", "Sneaking To Quit Earlier", "Master LV.1"), 4, Wdcskillsinv);
			itemset("Incantation", Material.TOTEM_OF_UNDYING, 0, 1, Arrays.asList(ChatColor.AQUA+"LV."+wsd.Incantation.getOrDefault(p.getUniqueId(), 0),"","RightClick + Sneaking", "Holding Totem", "Master LV.1"), 5, Wdcskillsinv);
			...
			if(Proficiency.getpro(p)<1) {
				...
				itemset("ZoglinCharge", Material.ZOGLIN_SPAWN_EGG, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"[Poison]","Use ZoglinCharge When Use Once More","(Damage Affected By Fangs)","",ChatColor.BOLD+" X (0.7D + "+BigDecimal.valueOf(wsd.Fangs.getOrDefault(p.getUniqueId(), 0)*0.8).setScale(2, RoundingMode.HALF_EVEN)+")"), 9, Wdcskillsinv);
				...
				itemset("PhantomSwoop", Material.BAT_SPAWN_EGG, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"[Poison]","Use PhantomSwoop When Use Once More","(Damage Affected By Wraith)","",ChatColor.BOLD+"20 X (0.2D + "+BigDecimal.valueOf(wsd.Wraith.getOrDefault(p.getUniqueId(), 0)*0.2).setScale(2, RoundingMode.HALF_EVEN)+")"), 12, Wdcskillsinv);
				...
				itemset("MarkOfDeath(Locked)", Material.STRUCTURE_VOID, 0, 1, Arrays.asList("Required Proficiency: "+ Proficiency.getproexp(p) + "/155015"), 26, Wdcskillsinv);
			}
			else {
				itemset("ZoglinCharge", Material.ZOGLIN_SPAWN_EGG, 0, 1, Arrays.asList(ChatColor.UNDERLINE+"[Poison]","Use ZoglinCharge When Use Once More","(Damage Affected By Fangs)","",ChatColor.BOLD+" X (0.7D + "+BigDecimal.valueOf(wsd.Fangs.getOrDefault(p.getUniqueId(), 0)*0.8).setScale(2, RoundingMode.HALF_EVEN)+")"), 9, Wdcskillsinv);
				itemset("ForbiddenHex", Material.NETHER_BRICK, 0, 1, Arrays.asList("Use ForbiddenHex When Use Once More","Consume 20% of Each Party Health", "Give Regeneration Effect", "Set Party Invulnerable for"+ChatColor.BOLD+BigDecimal.valueOf(wsd.Bosou.getOrDefault(p.getUniqueId() ,0)*0.5).setScale(2, RoundingMode.HALF_EVEN)+ "secs") , 10, Wdcskillsinv);
				...
				itemset("MarkOfDeath", Material.WRITTEN_BOOK, 0, 1, Arrays.asList("Sprinting + ThrowItem","Hold And Mark Near By Enemies","Enemies Will Get No damage", "And Prevent From Death For 4secs","","After 4secs, Mark Will Disappear And", 
						"Enemies will Get Increased Damage That","Before They Have got for 4secs","",ChatColor.BOLD+" X "+BigDecimal.valueOf((1+ p.getLevel()*0.001)).setScale(4, RoundingMode.HALF_EVEN)), 26, Wdcskillsinv);
			}
