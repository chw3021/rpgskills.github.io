...
public class RedRaids extends Mobs implements Listener {
	...
	private void RaidFinish(String rn, String title, String sub, Integer factor) {
    	...
    	heroes.get(rn).forEach(pu -> {
    		...
    		if(factor ==0) {
            	...
            	Elements.give(Elements.getel(10,p), 30/(heroes.get(rn).size()), p);
	...
	public void RedKnightRaidStart(PlayerInteractEvent d) 
	{	
		if(d.getClickedBlock() != null && d.getClickedBlock().hasMetadata("RedKnightRaidPortal") && d.getAction() == Action.RIGHT_CLICK_BLOCK && d.getAction() != Action.LEFT_CLICK_BLOCK) {
				...
				if(raidcool.containsKey(p.getUniqueId())) {
		            ...
		            {
						if(p.getLocale().equalsIgnoreCase("ko_kr")) {
			            	p.spigot().sendMessage(ChatMessageType.CHAT, new ComponentBuilder("붉은기사 침공작전 입장까지 " + timer + "초 기다려야 됩니다").create());
						}
						else {
			            	p.spigot().sendMessage(ChatMessageType.CHAT, new ComponentBuilder("You have to wait for " + timer + " seconds to Start RedKnightRaid").create());
						}
		            	return;
		            }
		            else { 
						if(p.getLocale().equalsIgnoreCase("ko_kr")) {
			            	p.spigot().sendMessage(ChatMessageType.CHAT, new ComponentBuilder("이제 붉은기사 침공작전에 입장하실수 있습니다").create());
						}
						else {
			            	p.spigot().sendMessage(ChatMessageType.CHAT, new ComponentBuilder("Now You Can Start RedKnightRaid").create());
						}
		            	raidcool.remove(p.getUniqueId());
		            }
				}
	    		World rw = Bukkit.getServer().getWorld("RedKnightRaid");
	    		...
	    		Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(RMain.getInstance(), new Runnable() {
	                ...
	                public void run() {
    		    		...
    		    		if(Party.hasParty(p)) {
	    	        		...
	    	        		portal.setMetadata("RedKnightRaidExit", new FixedMetadataValue(RMain.getInstance(), true));
	    	                ...
	    	                int rat = Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(RMain.getInstance(), new Runnable() {
	    		                ...
	    		                public void run() {
	    		            		...
	    		            		for(int i =0; i <19; i++) {
	    		    	        		...
	    		    	        		String reg = p.getLocale().equalsIgnoreCase("ko_kr") ? "붉은용사":"RedWarrior";
	    		        	    		Skeleton newmob = (Skeleton) MobspawnLoc(esl, ChatColor.RED+reg, 13000.0, head, chest, leg, boots, main, null, EntityType.SKELETON);
	    	        	    		...
	    	        	    		main.addUnsafeEnchantment(Enchantment.FIRE_ASPECT, 3);
	    	        	    		main.addUnsafeEnchantment(Enchantment.DAMAGE_ALL, 3);
    		    	        		String reg = p.getLocale().equalsIgnoreCase("ko_kr") ? "붉은단장":"RedWarlord";
	    	        	    		Skeleton newmob = (Skeleton) MobspawnLoc(esl, ChatColor.RED+reg, 40000.0, head, chest, leg, boots, main, off, EntityType.SKELETON);
	    	        	    		newmob.addPotionEffect(new PotionEffect(PotionEffectType.FIRE_RESISTANCE, 999999, 255, false, false));
	    	        	    		...
	    	        	    		BossBar	newbar = Bukkit.getServer().createBossBar(new NamespacedKey(RMain.getInstance(), p.getName()+"RedKnightRaid"),w, BarColor.RED, BarStyle.SEGMENTED_20);
	    	        	            newbar.setVisible(true);
	    	        	    		...
	    	        	    		BossBar	timeb = Bukkit.getServer().createBossBar(new NamespacedKey(RMain.getInstance(), p.getName()+"RedKnightRaidTime"),t + String.valueOf((int)timeout.get(p.getName())/20/60) + ":" + String.valueOf((int)(timeout.get(p.getName())/20)%60), BarColor.WHITE, BarStyle.SEGMENTED_6);
	...
	public void RedKnightRaidWave2(EntityDeathEvent d) 
	{	
                ...
                int rat =Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(RMain.getInstance(), new Runnable() {
	                ...
	                public void run() {
	            		for(int i =0; i <19; i++) {
	    	        		...
	    	        		String reg = Bukkit.getPlayer(rn).getLocale().equalsIgnoreCase("ko_kr") ? "붉은 하수인":"Red Hunter";
	    	        		Vindicator newmob = (Vindicator) MobspawnLoc(esl, ChatColor.RED+reg, 12000.0, head, chest, leg, boots, main, off, EntityType.VINDICATOR);
        	    		...
        	    		main.addUnsafeEnchantment(Enchantment.FIRE_ASPECT, 3);
        	    		main.addUnsafeEnchantment(Enchantment.DAMAGE_ALL, 5);
    	        		String reg = Bukkit.getPlayer(rn).getLocale().equalsIgnoreCase("ko_kr") ? "붉은 주술사":"RedWarlock";
        	    		Evoker newmob = (Evoker) MobspawnLoc(esl, ChatColor.RED+reg, 35000.0, head, chest, leg, boots, main, off, EntityType.EVOKER);
        	    		newmob.setCanJoinRaid(false);
        	    		newmob.setPatrolLeader(false);
        	    		newmob.setPatrolTarget(null);
        	    		newmob.addPotionEffect(new PotionEffect(PotionEffectType.FIRE_RESISTANCE, 999999, 255, false, false));
	...
	public void RedKnightRaidWave3(EntityDeathEvent d) 
	{	
		if(d.getEntity().hasMetadata("wave2") && raider.containsValue(d.getEntity().getUniqueId())) {
			...
			if(raider.get(rn).size()<=0){
            	...
            	heroes.get(rn).forEach(pu -> {
					if(Bukkit.getPlayer(pu).getLocale().equalsIgnoreCase("ko_kr")) {
	            		Bukkit.getPlayer(pu).sendTitle(ChatColor.GRAY + "2단계 완료", "10초뒤에 3번째 적들이 몰려옵니다", 5, 69, 5);
					}
					else {
	            		Bukkit.getPlayer(pu).sendTitle(ChatColor.RED + "Wave 2 Clear", "Wave 3 will be started after 10s", 5, 69, 5);
					}
            		Bukkit.getPlayer(pu).addPotionEffect(new PotionEffect(PotionEffectType.SLOW_FALLING, 150,1,false,false));
            		Bukkit.getPlayer(pu).teleport(spl.clone().add(0,2.5,0));
            	});
                ...
                int rat =Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(RMain.getInstance(), new Runnable() {
	                ...
	                public void run() {
	            		for(int i =0; i <15; i++) {
	        	    		...
	        	    		main.addUnsafeEnchantment(Enchantment.FIRE_ASPECT, 3);
	        	    		main.addUnsafeEnchantment(Enchantment.DAMAGE_ALL, 5);
	    	        		String reg = Bukkit.getPlayer(rn).getLocale().equalsIgnoreCase("ko_kr") ? "테라코타좀비":"WalkingTerracotta";
	        	    		Husk newmob = (Husk) MobspawnLoc(esl, ChatColor.RED+reg, 20000.0, head, chest, leg, boots, main, main, EntityType.HUSK);
	        	    		newmob.setAdult();
	        	    		newmob.addPotionEffect(new PotionEffect(PotionEffectType.FIRE_RESISTANCE, 999999, 255, false, false));
	        	    		newmob.getEquipment().setBootsDropChance(0);
	        	    		newmob.getEquipment().setChestplateDropChance(0);
	        	    		...
	        	    		newmob.getEquipment().setLeggingsDropChance(0);
	        	    		newmob.setMetadata("raid", new FixedMetadataValue(RMain.getInstance(),rn));
	        	    		newmob.setMetadata("wave3", new FixedMetadataValue(RMain.getInstance(), true));
	        	    		newmob.setMetadata("red", new FixedMetadataValue(RMain.getInstance(), true));
	            		...
	            		for(int i =0; i <4; i++) {
	        	    		...
	        	    		main.addUnsafeEnchantment(Enchantment.FIRE_ASPECT, 3);
	        	    		main.addUnsafeEnchantment(Enchantment.DAMAGE_ALL, 5);
	    	        		String reg = Bukkit.getPlayer(rn).getLocale().equalsIgnoreCase("ko_kr") ? "용암괴수":"Lavager";
	        	    		Ravager newmob = (Ravager) MobspawnLoc(esl, ChatColor.RED+reg, 25000.0, head, chest, leg, boots, main, main, EntityType.RAVAGER);
	        	    		newmob.addPotionEffect(new PotionEffect(PotionEffectType.FIRE_RESISTANCE, 999999, 255, false, false));
	        	    		newmob.getEquipment().setBootsDropChance(0);
	        	    		newmob.getEquipment().setChestplateDropChance(0);
	        	    		newmob.getEquipment().setHelmetDropChance(0);
                    	...
                    	double number2 = (random.nextDouble()+1.5) * 5 * (random.nextBoolean() ? -1 : 1);
                    	Location esl = spl.clone().add(number, 2.5, number2);
                    	ItemStack head = new ItemStack(Material.RED_GLAZED_TERRACOTTA);
        				ItemStack chest = new ItemStack(Material.LEATHER_CHESTPLATE);
        				LeatherArmorMeta chm = (LeatherArmorMeta) chest.getItemMeta();
        				chm.setColor(Color.RED);
        				chest.setItemMeta(chm);
        	    		...
        	    		main.addUnsafeEnchantment(Enchantment.FIRE_ASPECT, 3);
        	    		main.addUnsafeEnchantment(Enchantment.DAMAGE_ALL, 5);
    	        		String reg = Bukkit.getPlayer(rn).getLocale().equalsIgnoreCase("ko_kr") ? "워킹글레이즈드":"WalkingGlazed";
        	    		Husk newmob = (Husk) MobspawnLoc(esl, ChatColor.RED+reg, 45000.0, head, chest, leg, boots, main, off, EntityType.HUSK);
        	    		newmob.setAdult();
        	    		newmob.addPotionEffect(new PotionEffect(PotionEffectType.FIRE_RESISTANCE, 999999, 255, false, false));
        				newmob.setGlowing(true);
        	    		newmob.getEquipment().setBootsDropChance(0);
        	    		...
        	    		newmob.setMetadata("leader", new FixedMetadataValue(RMain.getInstance(),true));
        	    		newmob.setMetadata("headthrower", new FixedMetadataValue(RMain.getInstance(), true));
        	    		newmob.getAttribute(Attribute.GENERIC_MOVEMENT_SPEED).setBaseValue(0.276);
        	    		newmob.getAttribute(Attribute.GENERIC_KNOCKBACK_RESISTANCE).setBaseValue(1);
        	    		newmob.setMetadata("rpgspawned", new FixedMetadataValue(RMain.getInstance(), true));
        	    		newmob.setRemoveWhenFarAway(false);
        	    		newmob.setConversionTime(-1);
	...
	public void RedKnightRaidWave4(EntityDeathEvent d) 
	{	
		if(d.getEntity().hasMetadata("wave3") && raider.containsValue(d.getEntity().getUniqueId())) {
			LivingEntity le = d.getEntity();
			String rn = le.getMetadata("raid").get(0).asString();
			raider.remove(rn, le.getUniqueId());
			if(raider.get(rn).size()<=0){
            	...
            	heroes.get(rn).forEach(pu -> {
					if(Bukkit.getPlayer(pu).getLocale().equalsIgnoreCase("ko_kr")) {
	            		Bukkit.getPlayer(pu).sendTitle(ChatColor.GRAY + "3단계 완료", "10초뒤에 4번째 적들이 몰려옵니다", 5, 69, 5);
					}
					else {
	            		Bukkit.getPlayer(pu).sendTitle(ChatColor.RED + "Wave 3 Clear", "Wave 4 will be started after 10s", 5, 69, 5);
					}
            		Bukkit.getPlayer(pu).addPotionEffect(new PotionEffect(PotionEffectType.SLOW_FALLING, 150,1,false,false));
            		Bukkit.getPlayer(pu).teleport(spl.clone().add(0,2.5,0));
            	});
                ...
                int rat = Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(RMain.getInstance(), new Runnable() {
	                ...
	                public void run() {
	            		for(int i =0; i <19; i++) {
	    	        		...
	    	        		String reg = Bukkit.getPlayer(rn).getLocale().equalsIgnoreCase("ko_kr") ? "타오르는해골":"BurningSkull";
	        	    		Skeleton newmob = (Skeleton) MobspawnLoc(esl, ChatColor.RED+reg, 10000.0, null, chest, leg, boots, main, null, EntityType.SKELETON);
	        	    		newmob.addPotionEffect(new PotionEffect(PotionEffectType.FIRE_RESISTANCE, 999999, 255, false, false));
	        	    		newmob.getEquipment().setBootsDropChance(0);
	        	    		newmob.getEquipment().setChestplateDropChance(0);
	        	    		newmob.getEquipment().setHelmetDropChance(0);
	        	    		...
	        	    		newmob.getEquipment().setLeggingsDropChance(0);
	        	    		newmob.setMetadata("raid", new FixedMetadataValue(RMain.getInstance(),rn));
	        	    		newmob.setMetadata("wave4", new FixedMetadataValue(RMain.getInstance(), true));
	        	    		newmob.setMetadata("red", new FixedMetadataValue(RMain.getInstance(), true));
	        	    		newmob.setMetadata("arch", new FixedMetadataValue(RMain.getInstance(), true));
	        	    		newmob.getAttribute(Attribute.GENERIC_MOVEMENT_SPEED).setBaseValue(0.34);
	        	    		newmob.setMetadata("rpgspawned", new FixedMetadataValue(RMain.getInstance(), true));
	        	    		newmob.setRemoveWhenFarAway(false);
	        	    		newmob.setTarget(Bukkit.getPlayer(rn));
	        	    		newmob.setLootTable(null);
        	    		...
        	    		main.addUnsafeEnchantment(Enchantment.ARROW_KNOCKBACK, 2);
        	    		main.addUnsafeEnchantment(Enchantment.ARROW_FIRE, 5);
    	        		String reg = Bukkit.getPlayer(rn).getLocale().equalsIgnoreCase("ko_kr") ? "그을린궁수":"BurnedArcher";
        	    		Skeleton newmob = (Skeleton) MobspawnLoc(esl, ChatColor.RED+reg, 33000.0, head, chest, leg, boots, main, off, EntityType.SKELETON);
        	    		newmob.addPotionEffect(new PotionEffect(PotionEffectType.FIRE_RESISTANCE, 999999, 255, false, false));
        				newmob.setGlowing(true);
        	    		newmob.getEquipment().setBootsDropChance(0);
        	    		newmob.getEquipment().setChestplateDropChance(0);
	...
	public void RedKnightRaidBoss(EntityDeathEvent d) 
	{	
		if(d.getEntity().hasMetadata("wave4") && raider.containsValue(d.getEntity().getUniqueId())) {
			LivingEntity le = d.getEntity();
			String rn = le.getMetadata("raid").get(0).asString();
			raider.remove(rn, le.getUniqueId());
			if(raider.get(rn).size()<=0){
            	...
            	heroes.get(rn).forEach(pu -> {
					if(Bukkit.getPlayer(pu).getLocale().equalsIgnoreCase("ko_kr")) {
	            		Bukkit.getPlayer(pu).sendTitle(ChatColor.GRAY + "4단계 완료", "10초뒤에 보스 토벌 작전이 시작됩니다", 5, 69, 5);
					}
					else {
	            		Bukkit.getPlayer(pu).sendTitle(ChatColor.RED + "Wave 4 Clear", "Boss Wave will be started after 10s", 5, 69, 5);
					}
            		Bukkit.getPlayer(pu).addPotionEffect(new PotionEffect(PotionEffectType.SLOW_FALLING, 150,1,false,false));
            		Bukkit.getPlayer(pu).teleport(spl.clone().add(0,2.5,0));
            	});
        	    		...
        	    		int task = Bukkit.getServer().getScheduler().scheduleSyncRepeatingTask(RMain.getInstance(), new Runnable() {
        	                ...
        	                public void run() 
        	                {
        						...
        						if(Holding.ale(newmob)!=null) {
            	                	raidbar.get(rn).setProgress(Holding.ale(newmob).getHealth()/80000);
	...
	public void RedKnightRaidExit(EntityDamageByEntityEvent d) 
	{	
		if(d.getEntity().hasMetadata("RedKnightRaidExit")) {
			d.setCancelled(true);
	...
	public void RedKnightRaidExit(PlayerJoinEvent ev) 
	{
		Player p = ev.getPlayer();
	...
	public void RedKnightRaidExit(PluginEnableEvent ev) 
	{
	...
	public void RedKnightRaidExit(PluginDisableEvent ev) 
	{
		List<World> worlds = Bukkit.getServer().getWorlds();
		worlds.forEach(w -> w.getPlayers().forEach(b -> {
				Player p = (Player) b;
	...
	{	
		if(d.getEntity().hasMetadata("raid") && d.getEntity().hasMetadata("red") && !d.isCancelled()) {
	...
	public void Teleport(PlayerTeleportEvent e)
	{
		...
		if(e.getTo().getWorld().getName().contains("RedKnightRaid")) {
			if(!heroes.containsValue(p.getUniqueId())) {
				e.setCancelled(true);
			}
		}
		if(e.getFrom().getWorld().getName().contains("RedKnightRaid") && !e.getTo().getWorld().getName().contains("RedKnightRaid") && !e.getTo().getWorld().getName().contains("FakeDimension")) {
			if(heroes.containsValue(p.getUniqueId())) {
				e.setCancelled(true);
			}
		}
