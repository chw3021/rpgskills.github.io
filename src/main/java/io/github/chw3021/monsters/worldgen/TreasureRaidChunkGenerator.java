...
public class TreasureRaidChunkGenerator extends ChunkGenerator {
    ...
    public void generateSurface(@NotNull WorldInfo worldInfo, @NotNull Random random, int chunkX, int chunkZ, @NotNull ChunkGenerator.ChunkData chunkData) {
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                int height = worldInfo.getMinHeight()+6;
                chunkData.setBlock(x, height, z, material);
                for (int i = height-1; i > worldInfo.getMinHeight(); i--)
                	chunkData.setBlock(x, i, z, Material.IRON_BLOCK);
            }
        }
    }
    ...
    public List<BlockPopulator> getDefaultPopulators(World world) {
        return Arrays.asList((BlockPopulator)new GoldTrees());
    }
