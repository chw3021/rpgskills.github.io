...
public class GoldTrees extends BlockPopulator {
	@Override
	public void populate(World world, Random random, Chunk chunk) {
		if (random.nextBoolean()) {
		    ...
		    for (int i = 1; i < amount; i++) {
		        ...
		        chunk.getBlock(X, Y, Z).setType(Material.BUDDING_AMETHYST);
		        chunk.getBlock(X, Y+1, Z).setType(Material.LARGE_AMETHYST_BUD);
